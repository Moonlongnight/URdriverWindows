...
//获得合理的td序列
vector<double> Normal_LSPB_get_td(vector<double> td_seq, double a_max) {
	...
	vvd_temp.resize(td_sol.size());
	td_sol[0].push_back(td[0]);
	td_sol[0].push_back(td[1]);
	for (int i = 1; i < td_sol.size(); i++) {
		td_sol[i].push_back(-1);//td_sol[][0]=-1用以表示没有此td_sol
	}
	for (int i = 0; i < vvd_temp.size(); i++) {
		vvd_temp[i].push_back(-1);//vvd_temp[][0]=-1
	}

	vector<double> sumOFtd(td_sol.size());//记录各td_sol对应的Σtd
	vector<double> sumOF_vvd_temp(td_sol.size());
	...
	for (int i = 2; i < qsize - 2; i++) {//求td[i]
		for (int j = 0; j < td_seq.size(); j++) {//为0.016和0.032依次找Σtd最小序列
			//按照Σtd从小到大，依次分析“各线路td_sol[k]”在“td[i]=td_seq[j]时”，各轴加速度是否小于a_max
			//if td_sol[k][0]!=-1 && 各轴|a[i]|<a_max，then vvd_temp[j] = td_sol[k]+td_seq[j] as td_seq[j]'s minΣtd sol
			vvd_temp[j][0] = -1;
			for (int k = 0; k < td_sol.size(); k++) {
				if (td_sol[k][0] != -1) {
					i_temp = 0;//轴1
					if ((absd(d_temp=(((q[i+1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {//判断轴1的|a|是否<=a_max
						i_temp = 1;//轴2
						if ((absd(d_temp = (((q[i + 1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {
							i_temp = 2;//轴3
							if ((absd(d_temp = (((q[i + 1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {
								i_temp = 3;//轴4
								if ((absd(d_temp = (((q[i + 1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {
									...
									if ((absd(d_temp = (((q[i + 1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {
										...
										if ((absd(d_temp = (((q[i + 1][i_temp] - q[i][i_temp]) / td_seq[j] - (q[i][i_temp] - q[i - 1][i_temp]) / td_sol[k][i - 1]) / t[i]))) <= a_max) {
											vvd_temp[j] = td_sol[k];
											vvd_temp[j].push_back(td_seq[j]);
											sumOF_vvd_temp[j] = sumOFtd[k] + td_seq[j];
											break;
										}
									}
								}
		...
		//将vvd_temp内容拷回给td_sol，期间维系各td_sol随着下标从小到大，其Σtd是递增的
		i_temp = 0;
		for (int j = 0; j < vvd_temp.size(); j++) {//得到vvd_temp有i_temp个无效方案
			if (vvd_temp[j][0] == -1)
				i_temp++;
		}
		for (int j = 0; j < i_temp; j++) {//按下标从大到小，给td_sol赋值i_temp个td_sol[][0]=-1
			td_sol[td_sol.size() - 1 - j][0] = -1;
		}
		for (int j = 0; j < td_sol.size()-i_temp; j++) {//下标从小到大，给剩下的td_sol赋值
			d_temp = DBL_MAX;//min value
			d_temp2 = 0;//min index
			for (int k = 0; k < vvd_temp.size(); k++)//find cur min
			{
				if (vvd_temp[k][0] != -1 && sumOF_vvd_temp[k] < d_temp) {
					d_temp = sumOF_vvd_temp[k];
					d_temp2 = k;
				}
			}
			assert(d_temp != DBL_MAX);//进入这个循环，说明还有vvd_temp[][i]!=-1的，因此必然能找到一个cur min
			td_sol[j] = vvd_temp[d_temp2];
			sumOFtd[j] = sumOF_vvd_temp[d_temp2];
			vvd_temp[d_temp2][0] = -1;
		}
		assert(td_sol[0][0] != -1);//若为-1，表示在当前td_seq下无解
	}
	//简单起见，最后一个td[i]直接赋值为0.032即可
	td_sol[0].push_back(0.032);
	assert(td_sol[0].size() == qsize - 1);
...
//LSPB_6D_use_td
string Normal_LSPB_6D_use_td(vector<double> td) {
	int i_temp;
	double d_temp;
	vector<double> vd_temp;
